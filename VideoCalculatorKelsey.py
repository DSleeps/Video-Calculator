...
def calculate_settings(event):
    ...
    settings = {}
    if (preserve.get() == 0):
        cur_video_height = video_height * (1 - float(p1_entry.get())/100. - float(p2_entry.get())/100.)
...
def calculate_anchor(event):
    top_pix = float(top_entry.get())/100. * video_height
    bot_pix = float(bot_entry.get())/100. * video_height
    lef_pix = float(lef_entry.get())/100. * video_width
    rig_pix = float(rig_entry.get())/100. * video_width
...
gap_label = tk.Label(text="Size of the gap between videos")
...
# Output Window
output_window = tk.Text(height=8)
...
# Output Window
anchor_window = tk.Text(height=8)
